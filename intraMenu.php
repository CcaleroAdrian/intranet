		...
		<?php  
			...
			If ( !($opMenu <= 0) ) { 
				foreach ( $opMenu as $reg  )  {
					...
					echo ' <li style="padding:10px;"><i class="'.$reg["icono"].'"> ' .utf8_decode($reg["descripcion"]).'</i><ul style="z-index:4;">';
					foreach ($opSubMenu as $value) {
					 echo '<li><a href="'.$value["href"].'" >'.utf8_encode($value["descripcion"]).'</a></li>';
					}
